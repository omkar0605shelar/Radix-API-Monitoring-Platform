...
export class AlertCorrelationEngine {
    static recentAlerts = [];
    static CORRELATION_WINDOW_MS = 5 * 60 * 1000; // 5 minutes
    /**
     * Evaluates an incoming alert to see if it should correlate to an existing open incident
     */
    static async correlateAlert(alert) {
        const now = Date.now();
        // Purge alerts older than window
        this.recentAlerts = this.recentAlerts.filter(a => now - a.timestamp.getTime() <= this.CORRELATION_WINDOW_MS);
        this.recentAlerts.push(alert);
        // Count related alerts for this project/endpoint
        const correlated = this.recentAlerts.filter(a => a.projectId === alert.projectId &&
