...
export class IncidentAnalysisService {
   ...
   */
  static async analyzeIncident(incidentId: string): Promise<AIAnalysisResult> {
    ...
    const result: AIAnalysisResult = validated.success 
      ? {
          ...validated.data,
    ...
    const analysisRecord = await prisma.incidentAnalysis.create({
      data: {
        ...
        recommendations: result.recommendedActions as any,
