...
import { AIAnalysisResult, IncidentCategoryType, IncidentSeverityType } from '../types.js';
...
export interface IAIProvider {
  name: string;
  analyzeIncident(context: IncidentContext): Promise<AIAnalysisResult>;
}
...
export class NvidiaAIProvider implements IAIProvider {
  ...
  async analyzeIncident(context: IncidentContext): Promise<AIAnalysisResult> {
...
`;
    ...
    return {
      ...
      severity: (parsed.severity as IncidentSeverityType) || context.severity,
      category: (parsed.category as IncidentCategoryType) || 'APPLICATION',
...
export class FallbackAIProvider implements IAIProvider {
  ...
  async analyzeIncident(context: IncidentContext): Promise<AIAnalysisResult> {
    const startTime = Date.now();
    let category: IncidentCategoryType = 'APPLICATION';
