...
import { AnomalyResult, MetricSnapshot, TelemetryDataPoint } from '../types.js';
import { SeverityEngine } from './severityEngine.js';

interface EndpointTelemetryBuffer {
  latencies: number[];
  statuses: number[];
  timeouts: number[];
  lastUpdated: number;
...
export class AnomalyDetectionService {
  private static bufferMap = new Map<string, EndpointTelemetryBuffer>();
  private static readonly MAX_BUFFER_SIZE = 50;

  /**
   * Record a single telemetry point and evaluate for anomalies
   */
  static recordTelemetry(point: TelemetryDataPoint) {
   ...
   */
  static async evaluateEndpoint(
    projectId: string, 
    endpointId?: string,
    overrideCurrent?: { latency: number; errorRate: number; timeoutRate: number }
    ...
    try {
      if (endpointId) {
        ...
        if (historyRecords.length >= 5) {
          const totalDuration = historyRecords.reduce((acc: number, r: { duration: number }) => acc + r.duration, 0);
          baselineLatency = Math.round(totalDuration / historyRecords.length);
          const errors = historyRecords.filter((r: { status: number }) => r.status >= 400).length;
