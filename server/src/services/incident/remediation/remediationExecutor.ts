...
export interface IRemediationProvider {
  execute(actionType: string, parameters?: Record<string, any>): Promise<RemediationExecutionResult>;
}

/**
 * Safe Simulation Provider for development and demonstration
 */
export class SimulationRemediationProvider implements IRemediationProvider {
  async execute(actionType: string, parameters?: Record<string, any>): Promise<RemediationExecutionResult> {
 ...
 */
export class RedisRemediationProvider implements IRemediationProvider {
  async execute(actionType: string, parameters?: Record<string, any>): Promise<RemediationExecutionResult> {
    const startTime = Date.now();
    const logs: string[] = [];
...
export class RemediationExecutor {
   ...
   */
  static async executeAction(remediationId: string, actorId: string = 'SYSTEM'): Promise<RemediationExecutionResult> {
    ...
    const provider: IRemediationProvider = isSimMode || action.action_type !== 'clear_cache'
    ...
    try {
      result = await provider.execute(action.action_type, (action.parameters as any) || {});
    } catch (err: any) {
      result = {
        success: false,
        actionType: action.action_type,
